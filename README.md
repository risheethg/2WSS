...
**Outward Sync (Local → Stripe):**
1. API request creates/updates customer
2. Customer saved to database + Event added to outbox (same transaction)
3. Worker processes outbox events → Kafka
4. Integration service consumes Kafka events → Stripe API
5. Stripe sends confirmation webhook

**Inward Sync (Stripe → Local):**
